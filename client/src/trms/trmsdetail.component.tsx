...
export default function TrmsDetailComponent(props: MatchProps) {
  ...
  return (
    <div className="col trms card" style={{ backgroundColor: '#96c0ca' }}>
      <div className="card-body" style={{ backgroundColor: '#96c0ca' }}>
        <p className="name">
          Submitted by {trms.name} on {trms.date_created}
        </p>
        <p className="event_location">
          Event: {trms.event_name} starts on {trms.event_start_date} ends on{' '}
          {trms.event_end_date}
        </p>
        <p className="event_location">Event type: {trms.event_type}</p>
        <p>
          Event Cost ${trms.event_cost}, Projected Reimbursement $
          {trms.pro_reimbursement}
        </p>
        <p className="attachments"> {trms.attachments} </p>
      </div>
      {(userContext.role === 'Employee' || userContext.role === 'Sup') &&
